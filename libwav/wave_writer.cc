...
BOOL WaveWriter::Initialize(const CHAR *wszFile, const BOOL bExtensibleFormat) {
  BOOL bRet = FALSE;
  this->m_extFormat = bExtensibleFormat;
  const UINT32 bHeaderLenght =
      bExtensibleFormat ? WAVE_HEAD_EXT_LEN : WAVE_HEAD_LEN;
  CLOSE_HANDLE_IF(m_hFile);

  m_hFile = CreateFile(wszFile, GENERIC_WRITE, FILE_SHARE_READ, NULL,
  ...
  if (!WriteFile(m_hFile, (LPCVOID)WavHeader, bHeaderLenght, &dwWritten, 0) ||
      dwWritten != bHeaderLenght) {
    IF_ERROR_RETURN(bRet);
  }
...
BOOL WaveWriter::WriteWaveData(const BYTE *pData, const DWORD dwLength) {
  ...
  if (!WriteFile(m_hFile, (LPCVOID)pData, dwLength, &dwWritten, 0) ||
      dwWritten != dwLength) {
    IF_ERROR_RETURN(bRet);
  }
...
BOOL WaveWriter::FinalizeHeader(WAVEFORMATEX *pwfx, const UINT32 uiFileLength) {
  BOOL bRet = FALSE;
  DWORD dwMove;
  DWORD dwWritten;
  const UINT32 bHeaderLenght =
      this->m_extFormat ? WAVE_HEAD_EXT_LEN : WAVE_HEAD_LEN;

  BYTE WavHeader[WAVE_HEAD_EXT_LEN];
  memset(WavHeader, 0, sizeof(WavHeader));

  if ((dwMove = SetFilePointer(m_hFile, 0, NULL, FILE_BEGIN)) ==
...
BOOL WaveWriter::SetWaveHeaderExt(WAVEFORMATEX *pwfx, const UINT32 uiDataLen,
                                  BYTE *head) {
  if (uiDataLen == 0)
    return FALSE;
  ...
  priff->cb =
      (uiDataLen * pwfx->nBlockAlign) + WAVE_HEAD_EXT_LEN - sizeof(RIFFCHUNK);
...
BOOL WaveWriter::SetWaveHeader(const WAVEFORMATEX *pwfx, const UINT32 uiDataLen,
                               BYTE *head) {
  if (uiDataLen == 0)
    return FALSE;
  ...
  priff->cb =
      (uiDataLen * pwfx->nBlockAlign) + WAVE_HEAD_LEN - sizeof(RIFFCHUNK);
  priff->fccListType = SWAP32('WAVE');
